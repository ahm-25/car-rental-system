  ...
  template: `
    <section class="flex flex-col gap-8">
      <header class="flex flex-wrap items-end justify-between gap-6">
        <div>
          <h1 class="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">{{ 'cars.title' | t }}</h1>
        ...
        <div class="flex items-center gap-4">
          @if (meta(); as m) {
            <span class="hidden sm:inline text-sm font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">
              ...
              {{ 'cars.of' | t }} <span class="font-bold text-brand-600 dark:text-brand-400">{{ m.total }}</span>
            </span>
          }
      ...
      >
        <div class="grid gap-6 md:grid-cols-4">
          <div class="md:col-span-2">
            <label for="search" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-2 px-1">{{ 'cars.filters.search' | t }}</label>
            <div class="relative">
              <div class="absolute inset-y-0 start-0 ps-4 flex items-center pointer-events-none text-slate-400 dark:text-slate-500">
          ...
          <div>
            <label for="brand" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-2 px-1">{{ 'cars.filters.brand' | t }}</label>
            <input
              id="brand"
              type="text"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label for="min_price" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-2 px-1 text-truncate">{{ 'cars.filters.min_day' | t }}</label>
              <input
                id="min_price"
                type="number"
                min="0"
            ...
            <div>
              <label for="max_price" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-2 px-1 text-truncate">{{ 'cars.filters.max_day' | t }}</label>
              <input
                id="max_price"
                type="number"
                min="0"
        ...
        <div class="relative overflow-x-auto min-h-[400px]">
          <table class="min-w-full text-sm text-left">
            <thead class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
              <tr>
                <th class="px-6 py-5 whitespace-nowrap">{{ 'cars.table.name' | t }}</th>
                <th class="px-6 py-5 whitespace-nowrap">{{ 'cars.table.brand_model' | t }}</th>
                <th class="px-6 py-5 text-end whitespace-nowrap">{{ 'cars.table.mileage' | t }}</th>
                <th class="px-6 py-5 text-end whitespace-nowrap">{{ 'cars.table.price_day' | t }}</th>
                <th class="px-6 py-5 whitespace-nowrap">{{ 'cars.table.added' | t }}</th>
                <th class="px-6 py-5 text-end whitespace-nowrap">{{ 'cars.table.actions' | t }}</th>
              </tr>
            </thead>
                  ...
                  <tr>
                    <td colspan="6" class="px-6 py-20">
                      <div class="flex flex-col items-center justify-center text-center">
                        ...
                        <p class="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">{{ 'cars.empty_title' | t }}</p>
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 max-w-sm">{{ 'cars.empty_subtitle' | t }}</p>
                      </div>
                    </td>
                  </tr>
                }
      ...
      >
        <div class="bg-white dark:bg-slate-900 border border-transparent dark:border-slate-800 rounded-2xl shadow-elevated max-w-md w-full p-6">
          <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">
            {{ 'cars.delete_confirm.title' | t: { name: c.name } }}
          </h3>
          <p class="text-sm text-slate-600 dark:text-slate-400 mb-6">
            {{ 'cars.delete_confirm.body' | t }}
          </p>
